...
/// - `RS_ZENOH_BUCKET_ROUTING`: How samples map to buckets. `static` (default) writes
///   everything to `RS_ZENOH_BUCKET`; `key-prefix` uses the first chunk of the key
///   expression as the bucket name (created on demand) and the rest as the entry name,
///   falling back to `RS_ZENOH_BUCKET` for single-chunk keys
...
pub struct ZenohApiConfig {
    ...
    /// In `key-prefix` routing mode this is the fallback for single-chunk keys.
    pub bucket: String,
...
pub enum ZenohBucketRouting {
    /// All data goes to the single configured bucket; the full key becomes the entry.
    #[default]
    Static,
    /// The first chunk of the key is the bucket (created on demand), the rest the entry.
    KeyPrefix,
}
