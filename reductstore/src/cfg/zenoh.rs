...
/// - `RS_ZENOH_BUCKET_MAX_BLOCK_RECORDS`: `max_block_records` applied to buckets the Zenoh runtime creates (default: 65536).
///   Existing buckets are left untouched; `RS_DEFAULTS_BUCKET_MAX_BLOCK_RECORDS` changes the default for all buckets instead.
/// - `RS_ZENOH_INGEST_WORKERS`: Number of sharded ingest workers (default: min(cores, 16)). Samples are
///   dispatched by key expression, so per-key order is preserved while distinct entries write in parallel
/// - `RS_ZENOH_INGEST_STATS`: When set, log per-stage ingest timing periodically
///
...
pub struct ZenohApiConfig {
    ...
    /// `max_block_records` used when the runtime creates a bucket on behalf of a block.
...
impl Display for ZenohApiConfig {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "enabled={}, config={}, config_path={}, max_block_records={}, tls={}, auth={}, subscribers={}, queryables={}",
            self.enabled,
...
mod tests {
    ...
    #[rstest]
    fn parses_max_block_records() {
        let cfg = parse(&[("RS_ZENOH_BUCKET_MAX_BLOCK_RECORDS", "100000")]);
    ...
    fn parses_invalid_max_block_records_falls_back_to_default(#[case] raw: &str) {
        let cfg = parse(&[("RS_ZENOH_BUCKET_MAX_BLOCK_RECORDS", raw)]);
        assert_eq!(
            cfg.max_block_records,
            Some(DEFAULT_BUCKET_MAX_BLOCK_RECORDS)
        );
    ...
    fn test_display() {
        let cfg = parse(&[
            ...
            ("RS_ZENOH_BUCKET_MAX_BLOCK_RECORDS", "4096"),
