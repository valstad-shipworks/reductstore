...
///
/// In static routing mode all queries target the fixed bucket configured via
/// `RS_ZENOH_BUCKET` and the full Zenoh key expression is the entry name. In
/// key-prefix mode the first chunk of the selector is the bucket and the rest
/// the entry name, mirroring the subscriber's write mapping.
