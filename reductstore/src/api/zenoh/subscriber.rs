...
impl StageTimer {
    ...
    fn finish(self) {
        ...
        if n.is_multiple_of(INGEST_STATS_EVERY) {
...
pub(crate) struct SubscriberPipeline {
    ...
    drops: DropCounter,
    new_bucket_settings: BucketSettings,
}
...
impl SubscriberPipeline {
    pub(crate) fn new(router: Arc<BucketRouter>, components: Arc<Components>) -> Self {
        SubscriberPipeline {
            ...
            new_bucket_settings: BucketSettings::default(),
        }
    }

    /// Sets `max_block_records` for buckets this pipeline creates on demand.
    pub(crate) fn with_max_block_records(mut self, max_block_records: Option<u64>) -> Self {
        self.new_bucket_settings.max_block_records = max_block_records;
        self
    }
    ...
    async fn ensure_bucket(&self, bucket: &str, may_create: bool) -> Result<(), IngestError> {
        ...
        if self.components.storage.get_bucket(bucket).await.is_err() {
            ...
            match self
                .components
                .storage
                .create_bucket(bucket, self.new_bucket_settings.clone())
                .await
            {
                Ok(_) => info!(
...
mod tests {
    ...
    #[rstest]
    #[tokio::test]
    async fn on_demand_bucket_uses_configured_max_block_records(
        ...
        .with_max_block_records(Some(65536));

        write(&pipeline, "/run_xyz/motion", 100).await.unwrap();
