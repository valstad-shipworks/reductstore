...
///
/// In static routing mode all data is written to the fixed bucket configured
/// via `RS_ZENOH_BUCKET` and the full Zenoh key expression becomes the entry
/// name. In key-prefix mode the first chunk of the key selects the bucket
/// (created on demand) and the remaining chunks become the entry name.
...
impl SubscriberPipeline {
    ...
    /// Makes sure `bucket` exists, creating it with default settings on first
    ...
    async fn ensure_bucket(&self, bucket: &str) -> Result<(), ReductError> {
        ...
        if self.components.storage.get_bucket(bucket).await.is_err() {
                ...
                .await
            {
                ...
                // Lost a create race with another writer: the bucket exists now.
...
mod tests {
    ...
    async fn key_prefix_routing_single_chunk_falls_back(#[future] keeper: Arc<StateKeeper>) {
        ...
        // "bucket-1" is pre-created by the test keeper and acts as the fallback.
        let bucket = components
            .storage
            .get_bucket("bucket-1")
            .await
