...
pub(crate) async fn run_session(
    ...
    match config.bucket_routing {
        ...
        // Buckets are derived from key prefixes and created on demand by the
        // subscriber; even the fallback bucket may never be needed.
...
/// Splits a comma-separated key expression list (zenoh key expressions have
/// no alternation, so matching disjoint prefixes needs one declaration each).
