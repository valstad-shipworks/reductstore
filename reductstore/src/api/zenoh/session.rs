...
pub(crate) async fn run_session(
    ...
    for router in subscriber_routers.iter().chain(queryable_routers.iter()) {
        if let Some(bucket) = router.declared_bucket() {
            if declared_buckets.insert(bucket.to_string()) {
                ensure_bucket_exists(&components, bucket, config.max_block_records).await?;
            }
        }
    }
    ...
    let workers = if config.subscribers.is_empty() {
        None
    } else {
        let worker_count = ingest_worker_count();
        info!("Starting {} Zenoh ingest workers", worker_count);
        Some(Arc::new(IngestWorkers::spawn(worker_count, &mut handles)))
    };

    for (block, router) in config.subscribers.iter().zip(subscriber_routers) {
        let pipeline = Arc::new(
            SubscriberPipeline::new(router, Arc::clone(&components))
                .with_max_block_records(config.max_block_records),
        );
        info!(
        ...
        let workers = Arc::clone(workers.as_ref().expect("workers exist with subscribers"));
        declared += spawn_subscriber_block(&session, block, pipeline, workers, &mut handles).await;
    }
...
struct IngestWorkers {
    txs: Vec<tokio::sync::mpsc::Sender<(Arc<SubscriberPipeline>, Sample)>>,
}

impl IngestWorkers {
    fn spawn(count: usize, handles: &mut Vec<JoinHandle<()>>) -> Self {
        let mut txs = Vec::with_capacity(count);
        for _ in 0..count {
            let (tx, mut rx) = tokio::sync::mpsc::channel::<(Arc<SubscriberPipeline>, Sample)>(
                INGEST_WORKER_QUEUE,
            );
...
mod tests {
    ...
    async fn ensure_bucket_exists_creates_bucket_with_configured_block_records(
        ...
        ensure_bucket_exists(&components, "zenoh-new", Some(65536))
            .await
            .unwrap();

        let bucket = components.storage.get_bucket("zenoh-new").await.unwrap();
    ...
    async fn ensure_bucket_exists_keeps_existing_bucket_settings(
        ...
        components
            .storage
            .create_bucket("zenoh-old", existing)
            .await
            .unwrap();

        ensure_bucket_exists(&components, "zenoh-old", Some(65536))
            .await
            .unwrap();

        let bucket = components.storage.get_bucket("zenoh-old").await.unwrap();
        let settings = bucket.upgrade().unwrap().settings().await.unwrap();
