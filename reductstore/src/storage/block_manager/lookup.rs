...
impl BlockManager {
    ...
    fn find_block_id(&self, start: u64) -> Result<u64, ReductError> {
        self.block_index.active_block_id_at(start).ok_or_else(|| {
    ...
    pub async fn load_block(&mut self, block_id: u64) -> Result<BlockRef, ReductError> {
        if let Some(block) = self.block_cache.get_read(&block_id) {
            return Ok(block);
        }
        ...
        let buf = match FILE_CACHE.read(&path, SeekFrom::Start(0)).await {
            ...
            Err(err) => {
                ...
                if self.cfg.role == InstanceRole::Replica && !FILE_CACHE.try_exists(&path).await? {
        ...
        let block = Arc::new(AsyncRwLock::new(block_from_disk.into()));
...
mod tests {
    ...
    #[rstest]
    #[tokio::test]
    async fn test_load_block_from_write_cache_skips_read_cache(
        #[future] block_manager: BlockManager,
        block_id: u64,
    ) {
        let mut block_manager = block_manager.await;
        block_manager.start_new_block(block_id, 1024).await.unwrap();
        assert!(block_manager.block_cache.get_write(&block_id).is_some());
    ...
    #[rstest]
    #[tokio::test]
    async fn test_load_block_keeps_arc_after_write_cache_eviction(
        #[future] block_manager: BlockManager,
        block_id: u64,
    ) {
        let mut block_manager = block_manager.await;
        let held = block_manager.start_new_block(block_id, 1024).await.unwrap();
        block_manager
            .start_new_block(block_id + 1, 1024)
            .await
            .unwrap();
        block_manager
            .start_new_block(block_id + 2, 1024)
            .await
            .unwrap();
        assert!(block_manager.block_cache.get_write(&block_id).is_none());
        assert!(block_manager.block_cache.read_cache_contains(&block_id));

        let loaded = block_manager.load_block(block_id).await.unwrap();
        assert!(Arc::ptr_eq(&held, &loaded));
    }

    #[rstest]
    #[tokio::test]
    async fn test_load_block_from_disk_populates_read_cache(
        #[future] block_manager: BlockManager,
        block_id: u64,
    ) {
        let mut block_manager = block_manager.await;
    ...
    async fn test_load_block_stale_index_self_heals(#[future] block_manager: BlockManager) {
        ...
        block_manager
            .index_mut()
            .update_block(block_id, |index_block| {
